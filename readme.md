...
## Basic Usage
1. First run **`make init`** inside root directory to create tempalte configs.
2. Then run **`make build`** inside root directory.
3. Then run **`make up`** to start up the project for first time.
4. Use/update environment variables from [**`.envs`**](https://github.com/ruddra/docker-django/blob/master/.envs) folder.
